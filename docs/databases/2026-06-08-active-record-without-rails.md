...
# Исследование базы данных через IRB с помощью ActiveRecord

Обычно для быстрого исследования PostgreSQL я использую `psql` или SQL-клиент. В этот раз решил попробовать другой подход: подключиться к существующей БД через ActiveRecord и работать с таблицами как с Ruby-объектами.
...
User.find_by(last_name: "Скляров")
=> #<User ...>
```

Я не описал ни одного поля класса `User`. ActiveRecord сам прочитал схему PostgreSQL и динамически создал методы для каждой колонки таблицы!

На скриншоте ниже видно, что пустой класс уже умеет получать структуру таблицы и выполнять запросы к PostgreSQL.
...
Для небольших исследовательских задач и изучения чужой базы данных связка irb + ActiveRecord + PostgreSQL оказалась неожиданно удобной. За несколько минут получаем интерактивную ORM-консоль без генерации Rails-приложения и без написания SQL вручную.
